...
export interface DataTableProps<T> {
  ...
  /** Stable row identity — required so selection survives refetches. */
  getRowId: (row: T) => string
  /** Heading shown on the mobile card for each row. */
  mobileTitle: (row: T) => ReactNode
  /** Optional secondary line under the mobile card heading. */
 ...
 *
 * From md up this is a real <table> with sticky-styled headers. Below md the
 * same columns are re-laid out as stacked cards with label-above-value, because
 * DESIGN.md forbids horizontal scrolling on the body. Both roles get both
 * presentations — the breakpoint depends on the viewport, never on who is
 * signed in.
 */
export function DataTable<T>({
  ...
  return (
    <>
      ...
      {/* Mobile — same data, stacked. Never a horizontally scrolling table. */}
