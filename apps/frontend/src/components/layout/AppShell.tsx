    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? // The magenta left edge is how you know where you are. It uses
              // accent-bright because the base accent only reaches 2.9:1 on plum.
              'bg-white/5 text-ink-inverted before:absolute before:inset-y-1.5 before:left-0 before:w-[3px] before:rounded-pill before:bg-accent-bright'
  ...
  return (
    <Tooltip.Provider delayDuration={300}>
      <div className="flex min-h-dvh bg-canvas">
        {/* Sidebar: full from lg, icon rail from md. */}
        <aside
          className={cn(
        ...
        >
          ...
          <nav aria-label="Main" className="flex-1 overflow-y-auto px-2 py-2 lg:px-3">
            <ul className="flex flex-col gap-1">
              {items.map((item) => (
                <li key={item.to}>
                  <span className="hidden lg:block"><SidebarLink item={item} compact={false} /></span>
                  <span className="lg:hidden"><SidebarLink item={item} compact /></span>
                </li>
              ))}
            </ul>
          </nav>
        ...
        <div className="flex min-w-0 flex-1 flex-col md:ml-rail lg:ml-sidebar">
          ...
          <main
            ...
            className={cn(
              ...
              // Clear of the mobile tab bar and the home indicator.
        ...
        {/* Mobile tab bar — 5 destinations max, the rest behind "More". */}
        <nav
          aria-label="Main"
              ...
              return (
                <li key={item.to} className="flex-1">
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        // `relative` anchors the active indicator below.
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        {isActive ? <span aria-hidden className="absolute top-0 h-[3px] w-10 rounded-pill bg-accent" /> : null}
