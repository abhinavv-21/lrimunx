...
interface OfflineContextValue {
  ...
  isOnline: boolean
  pending: number
  /** Queues a write for later delivery and returns the queue length. */
...
export function OfflineProvider({ children }: { children: ReactNode }) {
  ...
  // Kept in a ref so the online listener never closes over a stale value.
  ...
  const sync = useCallback(async () => {
    ...
    try {
      ...
      if (result.synced > 0) {
        ...
        // Server state changed underneath us — refetch what is on screen.
