...
export function AwardsSection({
  ...
  /**
   * Creates the four awards nearly every committee gives, as empty slots.
   * Sequential rather than parallel so a mid-way failure leaves a coherent
   * list instead of an arbitrary subset.
   */
  ...
  return (
    <Card>
      ...
      {/* One datalist for every title field in this section. */}
      ...
      {error ? (
        <p role="alert" className="mb-4 rounded-control border border-danger bg-danger-wash p-3 text-body-sm text-ink">
          {error}
        </p>
      ) : null}
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <Select
              id="new-award-winner"
              value={newDelegateId}
              onChange={(event) => setNewDelegateId(event.target.value)}
            >
