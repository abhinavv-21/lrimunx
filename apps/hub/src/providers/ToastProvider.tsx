...
const TONE_STYLES: Record<ToastTone, { wrapper: string; icon: typeof Info }> = {
  ...
  warning: { wrapper: 'border-warning bg-warning-wash text-warning', icon: AlertTriangle },
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<ToastContextValue>(
    () => ({
      ...
      warning: (title, description) => notify({ tone: 'warning', title, ...(description ? { description } : {}) }),
          ...
          return (
            ...
            >
              ...
              <Toast.Close
                ...
                className="grid size-tap shrink-0 md:size-9 place-items-center rounded-control text-ink-secondary transition-colors duration-micro hover:bg-surface-sunken"
              >
