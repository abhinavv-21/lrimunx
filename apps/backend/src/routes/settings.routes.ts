...
/**
 * Both roles read settings — the registration form link is useful to anyone
 * on the floor. Only an admin can change them.
 */
settingsRouter.get(
  '/',
...
settingsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const [key, value] of Object.entries(body)) {
      ...
      if (value === '') {
        // Clearing removes the row entirely rather than storing an empty string.
