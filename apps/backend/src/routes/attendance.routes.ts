...
/**
 * Attendance is the one write a CONTRIBUTOR shares with an ADMIN, so every
 * check-in is audited regardless of role — the desk needs to know who marked
 * a delegate present, not only when an admin did it.
 */
...
attendanceRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Re-scanning an already checked-in delegate is a no-op, not an error —
    // it happens constantly at a busy registration desk.
