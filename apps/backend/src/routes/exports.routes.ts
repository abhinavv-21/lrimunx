...
// Exports carry the full delegate roster including dietary and accessibility
// notes, so they are ADMIN-only.
...
async function buildTable(request: ExportRequest): Promise<ExportTable> {
  switch (request.dataset) {
    case 'delegates': {
      ...
      return {
        title: 'Delegates',
        // Header spellings match what the CSV importer accepts, so an export
        // can be edited in Sheets and imported straight back.
        columns: [
    ...
    case 'attendance': {
      ...
      return {
        ...
        // Committee first, then alphabetical — the order the desk checks people in.
        rows: [...delegates]
          .sort((a, b) => {
