...
import { auditEntityParams, auditQuery } from '../schemas/index.js'
...
auditRouter.get(
  ...
  validate(auditEntityParams, 'params'),
  asyncHandler(async (req, res) => {
    const { entityType, entityId } = req.params as unknown as { entityType: string; entityId: string }
