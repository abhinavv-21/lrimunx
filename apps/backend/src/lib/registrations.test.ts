...
describe('generateReference', () => {
  ...
  it('never emits a character an applicant could misread down the phone', () => {
    // I/1 and O/0 are the pairs that get transcribed wrong.
  ...
  it('spreads across the alphabet rather than favouring a corner of it', () => {
    ...
    // Every symbol should appear over 30k characters, and none should dominate.
  ...
  it('does not repeat itself over a large run', () => {
    ...
    // Over 32^6 codes the expected number of collisions in 5,000 draws is about
    // 0.01, so the margin here is slack rather than tolerance — a generator with
    // a short period or a stuck seed would land nowhere near this.
...
describe('isHoneypotTripped', () => {
  it('treats an absent or blank field as a real applicant', () => {
    ...
    // A browser can post the hidden input padded with whitespace.
    expect(isHoneypotTripped('   ')).toBe(false)
  })
